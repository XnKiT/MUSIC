...
@bot.on_message(self_or_contact_filter & filters.command("stream", prefixes=f"{HNDLR}"))
...
@bot.on_message(self_or_contact_filter & filters.command("play", prefixes=f"{HNDLR}"))
...
@bot.on_message(self_or_contact_filter & filters.command("stop", prefixes=f"{HNDLR}"))
...
async def help(client, m: Message):
   await m.reply(f"**🔰DEVELOPER🔰 ↗ `(c) XnKiT ™`** \n\n**🛠 HELP MENU** \n\n`{HNDLR}ping` \n`{HNDLR}play query` \n`{HNDLR}stream link` \n`{HNDLR}stop` \n`{HNDLR}pause` \n`{HNDLR}resume` \n`{HNDLR}update` \n`{HNDLR}restart`")
